...
```{r getdata, comment=NA, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r fixdata, comment=NA, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r latlongs, comment=NA, warning=FALSE, message=FALSE, cache=TRUE}
latlongs <- llply(res[1:20], function(x) noaa_stations(x, dataset="GHCND")$data$meta[c('id','latitude','longitude')])
...
```{r lookatdata, comment=NA, warning=FALSE, message=FALSE, cache=TRUE}
head(df2)
```
...
```{r plotit, comment=NA, warning=FALSE, message=FALSE, cache=TRUE, fig.width=10, tidy=FALSE}

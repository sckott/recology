...
```{r gettext, message=FALSE, warning=FALSE, comment=NA}
library(rplos)
out <- searchplos("*:*", fields='id,introduction', limit=20,
                  toquery=list('cross_published_journal_key:PLoSONE', 'doc_type:full'))
out$idshort <- sapply(out$id, function(x) strsplit(x, "\\.")[[1]][length(strsplit(x, "\\.")[[1]])], USE.NAMES=FALSE)
```

The result is a list of length `limit` defined in the previous call. 

```{r peek, message=FALSE, warning=FALSE, comment=NA}
nrow(out)
```
...
```{r tmit, message=FALSE, warning=FALSE, comment=NA}
library(tm)
...
```{r wordcloud, message=FALSE, warning=FALSE, comment=NA}
library(wordcloud)

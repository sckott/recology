...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "../public/img/2020-02-10-how-many-parameters/"
)
```
...
I ended up using `r NROW(df$pkg)` functions across `r length(unique(df$pkg))` packages
...
The mean number of arguments per function across all packages was `r round(mean(df$value), 1)`,
and the most common value was `r median(df$value)`. The maximum number of arguments was
`r max(df$value)`, and there were `r length(df$value[df$value == 0])` functions
(or `r round((length(df$value[df$value == 0])/length(df$value))*100, 1)`%) with zero
parameters.
...
with a mean of `r round(mean(df_means$mean_params), 1)` arguments, and the most common value at `r median(round(df_means$mean_params, 0))`.

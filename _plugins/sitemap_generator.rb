# This plugin was originally written by Michael Levin. I have made a few minor 
# changes to how it generates URLs for posts and pages.
# 
# 
...
module Jekyll
  ...
  EXCLUDED_FILES = ["atom.xml", "404.html", "application.scss"]

  # Any files that include posts, so that when a new post is added, the last
  # modified date of these pages should take that into account
  PAGES_INCLUDE_POSTS = ["articles/index.html"]
  ...
  class Post
    ...
    def location_on_server
      location = "#{MY_URL}#{url}".strip
      location.gsub(/\/index.html$/, "").gsub(/\/$/, "")
    end
  end
  ...
  class Page
    ...
    def location_on_server
      location = "#{MY_URL}#{@dir}#{(!@dir[/\/$/] and !url[/^\//]) ? '/' : ''}#{url}".strip
      location.gsub(/\/index.html$/, "").gsub(/\/$/, "")
    end
  end
  ...
  class SitemapGenerator < Generator
    ...
    VALID_CHANGE_FREQUENCY_VALUES = ["always", "hourly", "daily", "weekly",
      ...
      def generate(site)
        ...
        path = File.join(site.dest, SITEMAP_FILE_NAME)
